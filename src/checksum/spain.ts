...
class DefaultAlgorithm extends Algorithm {
  ...
  override readonly accepts = [Component.BANK_CODE, Component.BRANCH_CODE, Component.ACCOUNT_CODE];

  compute(components: string[]): string {
    const [bankCode, branchCode, accountCode] = components;
    const weights = [1, 2, 4, 8, 5, 10, 9, 7, 3, 6];
    const d1 = reconcile(11 - weighted(bankCode + branchCode, 11, weights.slice(2)));
